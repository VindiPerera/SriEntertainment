      ...
      <div class="fixed inset-0 z-10 flex items-center justify-center">
        ...
        >
          ...
          >
            ...
            >
              {{ form.errors[0] || "An error occurred while deleting the newspaper." }}
            </p>
          </DialogPanel>
        </TransitionChild>
      </div>
...
const { open, selectedNewspaper } = defineProps({
  open: {
    type: Boolean,
    required: true,
  },
...
const deleteItem = () => {
  if (!selectedNewspaper?.id) {
    console.error("No newspaper selected for deletion");
    return;
  }

  form.delete(`/newspapers/${selectedNewspaper.id}`, {
    onSuccess: () => {
