...
<head>
    ...
    <title>Receipt - ${props.reloadSale.transaction_reference}</title>
    <style>
        @media print {
            body {
                margin: 0;
    ...
    <div class="receipt-container">
        ...
        <div class="section">
            <div class="info-row">
                <div class="info-item">
                    <p>Date:</p>
                    <small>${new Date(props.reloadSale.sale_date).toLocaleDateString()}</small>
                </div>
                <div class="info-item">
                    <p>Receipt No:</p>
                    <small>${props.reloadSale.transaction_reference || 'N/A'}</small>
                </div>
            </div>
            <div class="info-row">
                <div class="info-item">
                    <p>Operator:</p>
                    <small>${props.reloadSale.operator?.name || 'N/A'}</small>
                </div>
                <div class="info-item">
                    <p>Cashier:</p>
                    <small>${props.cashier?.name || props.reloadSale.user?.name || 'Staff'}</small>
                </div>
            </div>
            <div class="info-row">
                <div class="info-item">
                    <p>Mobile:</p>
                    <small>${props.reloadSale.msisdn || 'N/A'}</small>
                </div>
                <div class="info-item">
                    <p>Type:</p>
                    <small>Reload</small>
        ...
        <div class="section">
            <table>
                ...
                <tbody style="font-size:11px;">
                    <tr>
                        <td>Reload - ${f2(props.reloadSale.face_value)} LKR</td>
                        <td style="text-align:center;">1</td>
                        <td style="text-align:right;">${f2(props.reloadSale.face_value)} LKR</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="totals">
            ${props.reloadSale.card_surcharge && props.reloadSale.card_surcharge > 0 ? `
            <div style="font-size:12px; color:#666;">
                <span>Sub Total</span>
                <span>${f2(props.reloadSale.face_value)} LKR</span>
            </div>
            <div style="font-size:12px; color:#666;">
                <span>Card Surcharge (2.5%)</span>
                <span>${f2(props.reloadSale.card_surcharge)} LKR</span>
            </div>
            <div style="border-top: 1px solid #ccc; margin-top: 4px; padding-top: 4px;">
                <span>Total</span>
                <span>${f2(parseFloat(props.reloadSale.face_value) + parseFloat(props.reloadSale.card_surcharge))} LKR</span>
            </div>
            ` : `
            <div>
                <span>Total</span>
            ...
            `}
            ${props.reloadSale.payment_method ? `
            <div style="font-size:12px; margin-top:8px;">
                <span>Payment Method</span>
                <span style="text-transform:capitalize;">${props.reloadSale.payment_method}</span>
            </div>
            ` : ''}
            ${props.reloadSale.cash_received && props.reloadSale.cash_received > 0 ? `
            <div style="font-size:13px; margin-top:4px;">
                <span>Cash Received</span>
                <span>${f2(props.reloadSale.cash_received)} LKR</span>
            </div>
            <div style="font-size:13px; margin-top:4px; ${props.reloadSale.change_amount >= 0 ? 'color:#16a34a;' : 'color:#dc2626;'}">
                <span>Balance</span>
                <span>${props.reloadSale.change_amount >= 0 ? '' : '-'}${f2(Math.abs(props.reloadSale.change_amount || 0))} LKR</span>
            </div>
            ` : ''}
        </div>

        ${props.reloadSale.notes ? `
        <div class="section">
            <div style="margin-top:8px;">
                <strong>Notes:</strong> ${props.reloadSale.notes}
            </div>
        </div>
        ` : ''}
